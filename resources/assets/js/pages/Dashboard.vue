
<template>

<div class="col-md-9 col-sm-9">
      <div class="row">
        <div class="col-md-9 col-sm-9">

                <div id="main" class="">
                    <div class="white-box text-center chart-sec mb30">
                        <figure><img src="images/line-chart.jpg" class="img-fluid" /></figure>
                    </div>

              <div class="white-box dash_tab_sec">
                <h4 class="grey"><i class="fa fa-chart-line">&nbsp;</i>  Performance</h4>
                   <ul class="nav nav-tabs" role="tablist">
                      <li class="nav-item">
                        <a class="nav-link active" data-toggle="tab" href="#home">Queries</a>
                      </li>
                      <li class="nav-item">
                        <a class="nav-link" data-toggle="tab" href="#menu1">Pages</a>
                      </li>
                      <li class="nav-item">
                        <a class="nav-link" data-toggle="tab" href="#menu2">Countries</a>
                      </li>
                      <li class="nav-item">
                        <a class="nav-link" data-toggle="tab" href="#menu2">Devices</a>
                      </li>
                      <li class="nav-item">
                        <a class="nav-link" data-toggle="tab" href="#menu2">Search Apperance</a>
                      </li>
                    </ul>

                    <!-- Tab panes -->
                    <div class="tab-content">
                      <div id="home" class="container tab-pane active"><br>
                        
                        <div class="table-responsive">
                          <table class="table">
                            <thead>
                              <tr>
                                <th scope="col">Page</th>
                                <th scope="col">Clicks</th>
                                <th scope="col">Impression</th>
                                <th scope="col">CTR</th>
                              </tr>
                            </thead>
                            <tbody>
                              <tr>
                                <td scope="row">https://idata.com.np/contact</td>
                                <td>42</td>
                                <td>218</td>
                                <td>14.3%</td>
                              </tr>
                              
                              <tr>
                                <td scope="row">https://idata.com.np/contact</td>
                                <td>0</td>
                                <td>42</td>
                                <td>0.3%</td>
                              </tr>

                              <tr>
                                <td scope="row">https://idata.com.np/contact</td>
                                <td>0</td>
                                <td>30</td>
                                <td>0%</td>
                              </tr>

                              <tr>
                                <td scope="row">https://idata.com.np/contact</td>
                                <td>14</td>
                                <td>28</td>
                                <td>5%</td>
                              </tr>

                            </tbody>
                          </table>     
                        </div>

                      </div>
                      <div id="menu1" class="container tab-pane fade"><br>
                        <div class="table-responsive">
                          <table class="table">
                            <thead>
                              <tr>
                                <th scope="col">Page</th>
                                <th scope="col">Clicks</th>
                                <th scope="col">Impression</th>
                                <th scope="col">CTR</th>
                              </tr>
                            </thead>
                            <tbody>
                              <tr>
                                <td scope="row">https://idata.com.np/contact</td>
                                <td>42</td>
                                <td>218</td>
                                <td>14.3%</td>
                              </tr>
                              
                              <tr>
                                <td scope="row">https://idata.com.np/contact</td>
                                <td>0</td>
                                <td>42</td>
                                <td>0.3%</td>
                              </tr>

                              <tr>
                                <td scope="row">https://idata.com.np/contact</td>
                                <td>0</td>
                                <td>30</td>
                                <td>0%</td>
                              </tr>

                              <tr>
                                <td scope="row">https://idata.com.np/contact</td>
                                <td>14</td>
                                <td>28</td>
                                <td>5%</td>
                              </tr>

                            </tbody>
                          </table>     
                        </div>
                      </div>
                      <div id="menu2" class="container tab-pane fade"><br>
                        <div class="table-responsive">
                          <table class="table">
                            <thead>
                              <tr>
                                <th scope="col">Page</th>
                                <th scope="col">Clicks</th>
                                <th scope="col">Impression</th>
                                <th scope="col">CTR</th>
                              </tr>
                            </thead>
                            <tbody>
                              <tr>
                                <td scope="row">https://idata.com.np/contact</td>
                                <td>42</td>
                                <td>218</td>
                                <td>14.3%</td>
                              </tr>
                              
                              <tr>
                                <td scope="row">https://idata.com.np/contact</td>
                                <td>0</td>
                                <td>42</td>
                                <td>0.3%</td>
                              </tr>

                              <tr>
                                <td scope="row">https://idata.com.np/contact</td>
                                <td>0</td>
                                <td>30</td>
                                <td>0%</td>
                              </tr>

                              <tr>
                                <td scope="row">https://idata.com.np/contact</td>
                                <td>14</td>
                                <td>28</td>
                                <td>5%</td>
                              </tr>

                            </tbody>
                          </table>     
                        </div>
                      </div>
                    </div>

              </div>

          </div>
        </div>
        <div class="col-md-3 col-sm-3 pad-left-0">
            <div class="white-box mb20">
                <FollowSuggestionsList   :followSuggestion="followSuggestion"></FollowSuggestionsList>
            </div>
            <div class="clearfix"></div>
            <div id="sidebar2" class="white-box mb20">
            <div class="suggestions">
              <h4 class="grey"><i class="fa fa-star"></i>  Add to your feed</h4>
              <div class="follow-user">
                <img src="images/user-1.jpg" alt="" class="profile-photo-sm pull-left">
                <div>
                  <h5><a href="timeline.html">Sangita Dhital</a></h5>
                  <a href="#" class="text-green"><i class="fa fa-plus">&nbsp;</i> Follow</a>
                </div>
              </div>
              <div class="follow-user">
                <img src="images/user-2.jpg" alt="" class="profile-photo-sm pull-left">
                <div>
                  <h5><a href="timeline.html">Sagar Chapagain</a></h5>
                  <a href="#" class="text-green"><i class="fa fa-plus">&nbsp;</i> Follow</a>
                </div>
              </div>
              <div class="follow-user">
                <img src="images/user-3.jpg" alt="" class="profile-photo-sm pull-left">
                <div>
                  <h5><a href="timeline.html">Binita Thakur</a></h5>
                  <a href="#" class="text-green"><i class="fa fa-plus">&nbsp;</i> Follow</a>
                </div>
              </div>
              <div class="follow-user">
                <img src="images/user-4.jpg" alt="" class="profile-photo-sm pull-left">
                <div>
                  <h5><a href="timeline.html">Biswas Shrestha</a></h5>
                  <a href="#" class="text-green"><i class="fa fa-plus">&nbsp;</i> Follow</a>
                </div>
              </div>
              <div class="follow-user">
                <img src="images/user-6.jpg" alt="" class="profile-photo-sm pull-left">
                <div>
                  <h5><a href="timeline.html">Bikash Bhandari</a></h5>
                  <a href="#" class="text-green"><i class="fa fa-plus">&nbsp;</i> Follow</a>
                </div>
              </div>
            </div>
            </div>
            </div>
            <div class="clearfix"></div>
            </div>
</div>
</template>
<script>
import mixin  from './../mixins/LoadData.mixin.js';
import FollowSuggestionsList from './../components/Follows/FollowSuggestionsList';
import TheRightSideBar from './../components/TheRightSideBar';
    export default {
        mounted() {
            // console.log('Component mounted.')
        },
        mixins: [ mixin ],  
        data:function(){
          return {
            followSuggestion:''
          }
        },
        components:{
            TheRightSideBar,
            FollowSuggestionsList
        },
    }

   
</script>
